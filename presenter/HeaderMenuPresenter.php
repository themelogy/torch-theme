<?php
namespace Themes\Torch\Presenter;
...
class HeaderMenuPresenter extends Presenter
{
    ...
    {
        return PHP_EOL.'<ul class="nav flex-column flex-lg-row" id="mainNav">'.PHP_EOL;
    }
    ...
    public function getMenuWithDropDownWrapper($item)
    {
        return '<li class="dropdown">
		          <a class="dropdown-item dropdown-toggle '.$this->getActiveState($item, ' current').$this->getActiveStateOnChild($item, ' current').'" href="'.$item->getUrl().'">
				    '.$item->title.'
			      </a>
    ...
    {
        return '<li class="dropdown-submenu '.$this->getActiveStateOnChild($item, ' current').'">
		          <a class="dropdown-item dropdown-toggle" href="'.$item->getUrl().'">
					'.$item->getIcon().' '.$item->title.'
			      </a>
			      <ul class="dropdown-menu">
			      	'.$this->getChildMenuItems($item).'
